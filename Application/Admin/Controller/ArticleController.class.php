...
class ArticleController extends AdminController
{
     ...
     *
     * */
    public function setStat($model = 'Convenient'){
        return parent::setStatus('Convenient');
    }
    public function bianmin(){
        $map['status'] = 2;
        $list = $this->lists(M('Convenient'), $map);
        $this->assign('list', $list);
        $this->meta_title = '待审核';
        $this->display();
    ...
    * 首页/名企
    * */
    public function mingqi()
    {
        $list = M('company')
            ->where(array('jobinten is not null'))
            ->field('id,companyname,pay,jobinten,jobtitle,jobfield')
            ->select();
        $this->assign('list', $list);
        $this->display();
    }
    ...
    * */
    ...
    {
        $list = M('company')
            ->field('count(jobinten) num,jobinten')
            ->group('jobinten')
            ->select();
        $this->assign('list', $list);
        $this->display();
    ...
    * */
    ...
    {
        $list = M('company')
            ->where('jobfield is not null')
            ->field('count(jobfield) num,jobfield')
            ->group('jobfield')
            ->select();
        $this->assign('list', $list);
        $this->display();
    ...
    * */
    ...
    {
        $list = M('company')
            ->field('companyname,pay,jobinten,jobtitle')
            ->select();
        $this->assign('list', $list);
        $this->display();
    }
    ...
    * */
    ...
    {
        $list = M('resume')
            ->field('id,picture,name,gender,age,education,worktime,region,jobinten')
            ->select();
        $this->assign('list', $list);
        $this->display();
    }
    ...
    * */
    ...
    {
        $list = M('company')
            ->field('id,companyname,creditcode,name,mobile,site,companyprofile')
            ->select();
        $this->assign('list', $list);
        $this->display();
    }
    ...
    public function company_edit(){
        ...
        if(IS_POST){
            ...
            $datas=array(
                ...
                'jobinten'=>$data['jobinten'],
                'pay'=>$data['pay'],
                'jobtitle'=>$data['jobtitle'],
                'jobfield'=>$data['jobfield'],
                'fulltime'=>$data['fulltime'],
            );
            $res=M('company')->where($where)->save($datas);//更新修改的信息入库
            if($res){
        ...
        // 获取详细数据
        $Document = D('company');
        $data = $Document->where(array('id'=>$id))->find();
        if (!$data) {
            $this->error($Document->getError());
        }
        $this->assign('data', $data);
    ...
    {
        $list = M('personal')
            ->field('id,name,gender,mobile,birthday,education,major,worktime,email')
            ->select();
        $this->assign('list', $list);
        $this->display();
    }
